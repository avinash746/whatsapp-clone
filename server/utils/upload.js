...
const storage = new GridFsStorage({
    url: `mongodb://${username}:${password}@cluster0.l1p1u.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`,
    
    options: { useNewUrlParser: true, useUnifiedTopology: true }, // Added useUnifiedTopology
    file: (request, file) => {
        const match = ["image/png", "image/jpg"];

        // Corrected 'memeType' to 'mimetype'
        if(match.indexOf(file.mimetype) === -1) {
            return `${Date.now()}-blog-${file.originalname}`;
        }

        return {
            bucketName: "photos", // The name of the collection in MongoDB
            filename: `${Date.now()}-blog-${file.originalname}`
        };
    }
});
